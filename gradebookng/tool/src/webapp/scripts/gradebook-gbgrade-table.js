...
$(document).ready(function() {
  ...
  GbGradeTable.templates = {
    ...
    studentSummary: TrimPath.parseTemplate(
        $("#gradebookStudentSummaryTemplate").html().trim().toString()),
    gradeItemSummary: TrimPath.parseTemplate(
        $("#gradebookGradeItemSummaryTemplate").html().trim().toString())
  };
...
//    rowHeaders: GbGradeTable.studentCellRenderer,
  ...
  GbGradeTable.refreshStudentSummary();
  GbGradeTable.refreshGradeItemSummary();
};
...
GbGradeTable.refreshStudentSummary = function() {
  var $summary = $("#gradebookStudentSummary");
  var visible = GbGradeTable.instance.getData().length;
  var total = GbGradeTable.grades.length;

  $summary.html(GbGradeTable.templates.studentSummary.process());
  $summary.find(".gb-student-summary-counts .visible").html(visible);

  if (visible < total) {
    $summary.find(".gb-student-summary-counts").addClass("text-warning");
  }
};

GbGradeTable.refreshGradeItemSummary = function() {
  var $summary = $("#gradebookGradeItemSummary");
  var total = GbGradeTable.columns.length;
  var visible = GbGradeTable.instance.getSettings().columns.length - 2;

  $summary.html(GbGradeTable.templates.gradeItemSummary.process());
  $summary.find(".gb-item-summary-counts .visible").html(visible);
  $summary.find(".gb-item-summary-counts .total").html(total);

  if (visible < total) {
    $summary.find(".gb-item-summary-counts").addClass("text-warning");
  }
};